        ...
        <div class="col-10 col-sm-10 col-lg-10 mx-auto">
            <div class="aiz-titlebar text-left pb-5px">
                <div class="row align-items-center">
                    <div class="col-auto">
                        <h1 class="h3 fw-bold">{{ translate('All Measurement Points') }}</h1>
                    </div>
                </div>
            </div>
            <div class="card">
                <div
                    ...
                    <div class="table-tabs-container">
                        <ul class="nav nav-tabs border-0 " id="myTab" role="tablist">
                            @foreach ($measurement_point_tabs as $measurement_point_tab)
                                <li class="nav-item" role="presentation">
                                    <button class="nav-link px-0 pb-15px fs-14 fw-500 {{ $loop->first ? 'active' : '' }}"
                                        ...
                                        id="{{ Str::slug($measurement_point_tab) }}-tab"
                                        onclick="changeTab(this, '{{ Str::slug($measurement_point_tab) }}')" role="tab"
                                        aria-controls="{{ Str::slug($measurement_point_tab) }}">
                                        {{ translate($measurement_point_tab) }}
                                    </button>
                                </li>
                            @endforeach
                        </ul>
                    ...
                    <div class="">
                        @if (auth()->user()->can('add_measurement_points'))
                            <a href="javascript:void(0);" id="add_new_measurement_point"
                                class="position-relative overflow-hidden add-new-btn">
                                <span
                                    class="position-relative z-2 pr-15px fs-14 fw-500 text-blue label-text">{{ translate('Add New Measurement Point') }}</span>
                                <span
                ...
                <div class="tab-filter-bar">
                    <form class="" id="sort_measurement_points" action="" method="GET">
                            ...
                            <div class="flex-grow-1 mr-2">
                                <div class="input-group mb-0 border border-light px-3 bg-light rounded-1">
                                    ...
                                    <input type="text" class="form-control form-control-sm border-0 px-2 bg-transparent"
                                        ...
                                        placeholder="{{translate('Search Measurement Points ...')}}">
                                </div>
                            </div>
...
@section('script')
    <script type="text/javascript">
        let currentTab = '{{ Str::slug($measurement_point_tabs[0]) }}';
        var searchTimer;
        ...
        function sort_measurement_points(el) {
            $('#sort_measurement_points').submit();
        }

        function single_delete(measurementPointId) {
            $.ajax({
                url: "{{ route('measurement-points.destroy', ':id') }}".replace(':id', measurementPointId),
                type: 'GET',
                success: function (response) {
                    if (response == 1) {
                        AIZ.plugins.notify('success', '{{ translate('Selected item deleted successfully') }}');
                        hideBulkActionModal();
                        getMeasurementPoints(currentTab);
                    }
                }
            });
        }
        ...
        function bulk_delete() {
            var data = new FormData($('#sort_measurement_points')[0]);
            $.ajax({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                url: "{{route('bulk-measurement-points-delete')}}",
                type: 'POST',
                data: data,
                cache: false,
                contentType: false,
                ...
                success: function (response) {
                    if (response == 1) {
                        AIZ.plugins.notify('success', 'Selected measurement points Deleted successfully');
                        hideBulkActionModal();
                        getMeasurementPoints(currentTab);
                    }
                },
        ...
        function bulkDeleted() {
            if ($('.check-one:checked').length == 0) {
                AIZ.plugins.notify('danger', '{{ translate('Please select at least one measurement point') }}');
                return;
            }

            showBulkActionModal();
            $('#confirmation-title').text('{{ translate('Delete Confirmation') }}');
            $('#confirmation-question').text('{{ translate('Are you sure you want to delete the selected measurement points?') }}');
        ...
        function singleDelete(measurementPointId) {
            showBulkActionModal();
            $('#confirmation-title').text('{{ translate('Delete Confirmation') }}');
            $('#confirmation-question').text('{{ translate('Are you sure you want to delete the selected measurement point?') }}');
            $('#conform-yes-btn').attr("onclick", "single_delete(" + measurementPointId + ")");
            $('.confirmation-icon').addClass('d-none');
            $('#delete-confirm-icon').removeClass('d-none');
        }


        function getMeasurementPoints(slug, page = 1) {
            ...
            $.ajax({
                url: `{{ route('measurement-points.filter') }}?page=${page}`,
                method: 'GET',
                data: { status: status, color_status: slug, search: keyword },
                success: function (response) {
        ...
        function changeTab(button, statusSlug) {
            ...
            getMeasurementPoints(statusSlug);
        }
        ...
        function openRightcanvas() {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('measurement-points.create') }}",
        ...
        $(document).on('click', '#store-new-measurement-point', function () {
            const btn = $(this);
            ...
            $.ajax({
                url: "{{ route('measurement-points.store') }}",
                type: "POST",
                data: {
                ...
                success: function (res) {
                    AIZ.plugins.notify('success', 'Measurement Point has been inserted successfully');
                    closeRightcanvas();
                    getMeasurementPoints(currentTab);
                },
                error: function () {
        ...
        function edit_measurement_point(measurementPointId) {
            openRightcanvasShell();

            $.ajax({
                type: "POST",
                url: "{{ route('measurement-points.edit') }}",
                data: {
                    _token: AIZ.data.csrf,
        ...
        $(document).on('click', '#update-measurement-point', function () {
            const btn = $(this);
            const id = $('#edit_measurement_point_id').val();
            const name = $('#rightOffcanvas input[name="name"]').val().trim();
            ...
            $.ajax({
                url: "{{ route('measurement-points.update') }}",
                type: "POST",
                data: {
                    _token: AIZ.data.csrf,
                    id: id,
                ...
                success: function (res) {
                    if (res == 1) {
                        AIZ.plugins.notify('success', '{{ translate("Measurement Point updated successfully") }}');
                        closeRightcanvas();
                        getMeasurementPoints(currentTab);
                    }
                },
