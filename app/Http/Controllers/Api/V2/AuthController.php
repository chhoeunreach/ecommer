...
class AuthController extends Controller
{
    ...
    public function guestUserAccountCreate(Request $request)
    {
        ...
        if($request->has('billing_address') || ($request->has('set_billing') && $request->set_billing == 1)){
            $billing_address = new Address();
            $billing_address->user_id       = $user->id;
            $billing_address->address       = $request->billing_address ?? $request->address;
            $billing_address->country_id    = $request->billing_country_id ?? $request->country_id;
            $billing_address->state_id      = $request->billing_state_id ?? $request->state_id;
            $billing_address->city_id       = $request->billing_city_id ?? $request->city_id;
            $billing_address->postal_code   = $request->billing_postal_code ?? $request->postal_code;
            $billing_address->phone         = $request->billing_phone ?? $request->phone;
            $billing_address->longitude     = $request->billing_longitude ?? $request->longitude;
            $billing_address->latitude      = $request->billing_latitude ?? $request->latitude;
        ...
        Cart::where('temp_user_id', $request->temp_user_id)
            ->update([
                ...
                'billing_address' => isset($billing_address) ? $billing_address->id : null
            ]);
