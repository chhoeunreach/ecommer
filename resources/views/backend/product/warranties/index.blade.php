        ...
        <div class="col-10 col-sm-10 col-lg-10 mx-auto">
            <div class="aiz-titlebar text-left pb-5px">
                <div class="row align-items-center">
                    <div class="col-auto">
                        <h1 class="h3 fw-bold">{{ translate('All Warranties') }}</h1>
                    </div>
                </div>
            </div>
            <div class="card">
                <div
                    ...
                    <div class="table-tabs-container">
                        <ul class="nav nav-tabs border-0 " id="myTab" role="tablist">
                            @foreach ($warranty_tabs as $warranty_tab)
                                <li class="nav-item" role="presentation">
                                    <button class="nav-link px-0 pb-15px fs-14 fw-500 {{ $loop->first ? 'active' : '' }}"
                                        ...
                                        id="{{ Str::slug($warranty_tab) }}-tab"
                                        onclick="changeTab(this, '{{ Str::slug($warranty_tab) }}')" role="tab"
                                        aria-controls="{{ Str::slug($warranty_tab) }}">
                                        {{ translate($warranty_tab) }}
                                    </button>
                                </li>
                            @endforeach
                        </ul>
                    ...
                    <div class="">
                        @if (auth()->user()->can('add_product_warranty'))
                            <a href="javascript:void(0);" id="add_new_warranty"
                                class="position-relative overflow-hidden add-new-btn">
                                <span
                                    class="position-relative z-2 pr-15px fs-14 fw-500 text-blue label-text">{{ translate('Add New Warranty') }}</span>
                                <span
                ...
                <div class="tab-filter-bar">
                    <form class="" id="sort_warranties" action="" method="GET">
                            ...
                            <div class="flex-grow-1 mr-2">
                                <div class="input-group mb-0 border border-light px-3 bg-light rounded-1">
                                    ...
                                    <input type="text" class="form-control form-control-sm border-0 px-2 bg-transparent"
                                        id="search_input" name="search" placeholder="{{translate('Search Warranties ...')}}">
                                </div>
                            </div>
...
@section('script')
    <script type="text/javascript">
        let currentTab = '{{ Str::slug($warranty_tabs[0]) }}';
        var searchTimer;
        ...
        function sort_warranties(el) {
            $('#sort_warranties').submit();
        }

        function single_delete(warrantyId) {
            $.ajax({
                url: "{{ route('warranties.destroy', ':id') }}".replace(':id', warrantyId),
                type: 'GET',
                success: function (response) {
                    if (response == 1) {
                        AIZ.plugins.notify('success', '{{ translate('Selected item deleted successfully') }}');
                        hideBulkActionModal();
                        getWarranties(currentTab);
                    }
                }
            });
        }
        ...
        function bulk_delete() {
            var data = new FormData($('#sort_warranties')[0]);
            $.ajax({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                url: "{{route('bulk-warranty-delete')}}",
                type: 'POST',
                data: data,
                cache: false,
                contentType: false,
                ...
                success: function (response) {
                    if (response == 1) {
                        AIZ.plugins.notify('success', 'Selected warranties Deleted successfully');
                        hideBulkActionModal();
                        getWarranties(currentTab);
                    }
                },
        ...
        function bulkDeleted() {
            if ($('.check-one:checked').length == 0) {
                AIZ.plugins.notify('danger', '{{ translate('Please select at least one warranty') }}');
                return;
            }

            showBulkActionModal();
            $('#confirmation-title').text('{{ translate('Delete Confirmation') }}');
            $('#confirmation-question').text('{{ translate('Are you sure you want to delete the selected warranties?') }}');
        ...
        function singleDelete(warrantyId) {
            showBulkActionModal();
            $('#confirmation-title').text('{{ translate('Delete Confirmation') }}');
            $('#confirmation-question').text('{{ translate('Are you sure you want to delete the selected warranty?') }}');
            $('#conform-yes-btn').attr("onclick", "single_delete(" + warrantyId + ")");
            $('.confirmation-icon').addClass('d-none');
            $('#delete-confirm-icon').removeClass('d-none');
        }


        function getWarranties(slug, page = 1) {
            ...
            $.ajax({
                url: `{{ route('warranties.filter') }}?page=${page}`,
                method: 'GET',
                data: { status: status, warranty_status: slug, search: keyword },
                success: function (response) {
        ...
        $(document).on('click', '#store-new-warranty', function () {
            const btn = $(this);
            const text = $('#rightOffcanvas input[name="text"]').val().trim();
            const logo = $('#rightOffcanvas input[name="logo"]').val().trim();

            if (!text) {
                AIZ.plugins.notify('warning', 'Please fill text');
                return;
            }
            ...
            $.ajax({
                ...
                success: function (res) {
                    AIZ.plugins.notify('success', 'Warranty has been inserted successfully');
                    closeRightcanvas();
                    getWarranties(currentTab);
                },
                error: function () {
        ...
        function edit_warranty(warrantyId) {
            openRightcanvasShell();

            $.ajax({
                type: "POST",
                url: "{{ route('warranties.edit') }}",
                data: {
                    _token: AIZ.data.csrf,
                    id: warrantyId
                },
        ...
        $(document).on('click', '#update-warranty', function () {
            const btn = $(this);
            const id = $('#edit_warranty_id').val();
            const text = $('#rightOffcanvas input[name="text"]').val().trim();
            const logo = $('#rightOffcanvas input[name="logo"]').val().trim();
			const lang = $('#lang').val();

            if (!text) {
                AIZ.plugins.notify('warning', '{{ translate("Please fill text") }}');
                return;
            }
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (res == 1) {
                        AIZ.plugins.notify('success', '{{ translate("Warranty updated successfully") }}');
                        closeRightcanvas();
                        getWarranties(currentTab);
                    }
                },
		...
		$(document).on('click', '.warranty-lang-switch', function (e) {
            e.preventDefault();
