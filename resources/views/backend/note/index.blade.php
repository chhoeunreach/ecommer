        ...
        <div class="col-10 col-sm-10 col-lg-10 mx-auto">
            <div class="aiz-titlebar text-left pb-5px">
                <div class="row align-items-center">
                    <div class="col-auto">
                        <h1 class="h3 fw-bold">{{ translate('All Notes') }}</h1>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body">
                    <div class="form-group mb-0 row">
                        <div class="d-flex align-items-center mt-3 mb-2 ml-3">
                            ...
                            <span class="fs-14 fw-400 d-block" style="margin-top: -6px">{{ translate('Seller Can Add Note') }}?</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card">
                <div
                            ...
                            @foreach ($note_tabs as $note_tab)
                                <li class="nav-item" role="presentation">
                                    <button class="nav-link px-0 pb-15px fs-14 fw-500 {{ $loop->first ? 'active' : '' }}"
                                        ...
                                        id="{{ Str::slug($note_tab) }}-tab"
                                        onclick="changeTab(this, '{{ Str::slug($note_tab) }}')" role="tab"
                                        aria-controls="{{ Str::slug($note_tab) }}">
                                        {{ translate($note_tab) }}
                                    </button>
                                </li>
                            @endforeach
                    ...
                    <div class="">
                        @if (auth()->user()->can('add_note'))
                            <a href="javascript:void(0);" id="add_new_note"
                                class="position-relative overflow-hidden add-new-btn">
                                <span
                                    class="position-relative z-2 pr-15px fs-14 fw-500 text-blue label-text">{{ translate('Add New Note') }}</span>
                                <span
                            ...
                            <div class="flex-grow-1 mr-2">
                                <div class="input-group mb-0 border border-light px-3 bg-light rounded-1">
                                    ...
                                    <input type="text" class="form-control form-control-sm border-0 px-2 bg-transparent"
                                        id="search_input" name="search" placeholder="{{translate('Search Notes ...')}}">
                                </div>
                            </div>
...
@section('script')
    <script type="text/javascript">
        let currentTab = '{{ Str::slug($note_tabs[0]) }}';
        var searchTimer;
        ...
        function sort_notes(el) {
            $('#sort_notes').submit();
        }

        function single_delete(noteId) {
            $.ajax({
                url: "{{ route('note.delete', ':id') }}".replace(':id', noteId),
                type: 'GET',
        ...
        function bulk_delete() {
            var data = new FormData($('#sort_notes')[0]);
            $.ajax({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                url: "{{route('bulk-note-delete')}}",
                type: 'POST',
                data: data,
                cache: false,
                contentType: false,
                ...
                success: function (response) {
                    if (response == 1) {
                        AIZ.plugins.notify('success', 'Selected notes Deleted successfully');
                        hideBulkActionModal();
                        getNotes(currentTab);
                    }
                },
        ...
        function bulkDeleted() {
            if ($('.check-one:checked').length == 0) {
                AIZ.plugins.notify('danger', '{{ translate('Please select at least one note') }}');
                return;
            }

            showBulkActionModal();
            $('#confirmation-title').text('{{ translate('Delete Confirmation') }}');
            $('#confirmation-question').text('{{ translate('Are you sure you want to delete the selected notes?') }}');
        ...
        function singleDelete(noteId) {
            showBulkActionModal();
            $('#confirmation-title').text('{{ translate('Delete Confirmation') }}');
            $('#confirmation-question').text('{{ translate('Are you sure you want to delete the selected note?') }}');
            $('#conform-yes-btn').attr("onclick", "single_delete(" + noteId + ")");
            $('.confirmation-icon').addClass('d-none');
            $('#delete-confirm-icon').removeClass('d-none');
        }


        function getNotes(slug, page = 1) {
            ...
            $.ajax({
                url: `{{ route('note.filter') }}?page=${page}`,
                method: 'GET',
                data: { status: status, note_status: slug, search: keyword },
                success: function (response) {
        ...
        $(document).on('click', '#store-new-note', function () {
            const btn = $(this);
            const note_type = $('#rightOffcanvas [name="note_type"]').val().trim();
            const description = $('#rightOffcanvas [name="description"]').val().trim();

            if (!note_type) {
                AIZ.plugins.notify('warning', 'Please fill note type');
                return;
            }

            if (!description) {
                AIZ.plugins.notify('warning', 'Please fill description');
                return;
            }
            ...
            $.ajax({
                ...
                success: function (res) {
                    AIZ.plugins.notify('success', 'Note has been inserted successfully');
                    closeRightcanvas();
                    getNotes(currentTab);
                },
                error: function () {
        ...
        $(document).on('click', '#update-note', function () {
            ...
            if (!note_type) {
                AIZ.plugins.notify('warning', '{{ translate("Please fill note type") }}');
                return;
            }
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (res == 1) {
                        AIZ.plugins.notify('success', '{{ translate("Note updated successfully") }}');
                        closeRightcanvas();
                        getNotes(currentTab);
                    }
                },
		...
		$(document).on('click', '.note-lang-switch', function (e) {
            e.preventDefault();
