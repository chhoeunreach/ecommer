    ...
    <div class="page-content">
        <div class="aiz-titlebar text-left mt-2 pb-2 px-3 px-md-2rem">
            <div class="row align-items-center">
                <div class="col">
                    <h1 class="h3 fw-700">{{ translate('Edit Product') }}: {{ $upcoming_product->name }}</h1>
                </div>
            </div>
        </div>

        <div class="px-3 px-md-2rem mb-4">
            <form action="{{ route('admin.upcoming-products.update', $upcoming_product->id) }}" method="POST" id="aizSubmitForm">
                @csrf
                @method('PUT')
