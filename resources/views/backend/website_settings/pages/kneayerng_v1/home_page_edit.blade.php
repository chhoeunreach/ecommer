	...
	<div class="page-content">
		<div class="aiz-titlebar text-left mt-2 pb-2 px-3 px-md-2rem border-bottom border-gray">
			<div class="row align-items-center">
				<div class="col">
					<h1 class="h3">{{ translate('Homepage Settings (Kneayerng V1)') }}</h1>
				</div>
			</div>
		</div>
						...
						<form action="{{ route('business_settings.update') }}" method="POST" enctype="multipart/form-data">
							...
							<div class="bg-white p-3 p-sm-2rem">
								...
								@php
									...
									$selectedFeaturedProducts = App\Models\Product::where('featured', 1)
										->latest()
										->limit(12)
										->get();
								@endphp
								...
								<div class="form-group mb-0">
									<div class="d-flex align-items-center justify-content-between mb-3">
										<div>
											<label class="col-from-label mb-1">{{ translate('Featured Products') }}</label>
											<p class="text-muted fs-12 mb-0">{{ translate('Choose up to 12 products to display on the homepage.') }}</p>
										</div>
										<button type="button" class="btn btn-sm btn-primary" onclick="openFeaturedProductModal()">
											<i class="las la-plus mr-1"></i>{{ translate('Add Products') }}
										</button>
									</div>
									...
									<div class="table-responsive">
										<table class="table aiz-table mb-0">
											...
											<tbody id="selected-featured-products">
												@foreach ($selectedFeaturedProducts as $featuredProduct)
													<tr data-product-id="{{ $featuredProduct->id }}">
														<td>
															<div class="d-flex align-items-center">
																<img class="size-48px img-fit mr-3" src="{{ uploaded_asset($featuredProduct->thumbnail_img) }}"
																	alt="{{ $featuredProduct->getTranslation('name') }}"
																	onerror="this.onerror=null;this.src='{{ static_asset('assets/img/placeholder.jpg') }}';">
																<span class="fs-13 fw-600">{{ $featuredProduct->getTranslation('name') }}</span>
															</div>
															<input type="hidden" name="featured_product_ids[]" value="{{ $featuredProduct->id }}">
														</td>
														<td class="text-right">
															<button type="button" class="btn btn-icon btn-circle btn-sm btn-soft-danger remove-featured-product">
									...
									<div id="featured-products-empty" class="border border-dashed rounded text-center text-muted py-4 {{ $selectedFeaturedProducts->isNotEmpty() ? 'd-none' : '' }}">
										{{ translate('No featured products selected yet.') }}
									</div>
								</div>
								<!-- Save Button -->
								<div class="mt-4 text-right">
...
@section('modal')
	<div class="modal fade" id="featured-product-select-modal" tabindex="-1" role="dialog" aria-hidden="true">
		<div class="modal-dialog modal-lg modal-dialog-centered" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title h6">{{ translate('Add Featured Products') }}</h5>
					<button type="button" class="close" data-dismiss="modal" aria-label="{{ translate('Close') }}">
						<span aria-hidden="true">&times;</span>
					</button>
				</div>
				<div class="modal-body">
					<input type="text" class="form-control" id="featured-product-search"
						placeholder="{{ translate('Search by Product Name') }}">
					<div class="mt-3" id="featured-product-search-results"></div>
				</div>
				<div class="modal-footer">
					<button type="button" class="btn btn-light" data-dismiss="modal">{{ translate('Cancel') }}</button>
					<button type="button" class="btn btn-primary" onclick="addSelectedFeaturedProducts()">
...
@section('script')
	<script>
		...
		function selectedFeaturedProductIds() {
			return $('#selected-featured-products tr').map(function () {
		...
		function loadFeaturedProducts() {
			...
			featuredProductSearchTimer = setTimeout(function () {
				$('#featured-product-search-results').html(
					'<div class="text-center py-5"><i class="las la-spinner la-spin la-2x"></i></div>'
				);

				$.post('{{ route('website.featured-products.search') }}', {
					_token: '{{ csrf_token() }}',
					search_key: $('#featured-product-search').val()
				}, function (html) {
					$('#featured-product-search-results').html(html);
					selectedFeaturedProductIds().forEach(function (id) {
						$('#featured-product-search-results .product-select[value="' + id + '"]').prop('checked', true);
					});
				});
			}, 300);
		}
		...
		function openFeaturedProductModal() {
			$('#featured-product-search').val('');
			$('#featured-product-select-modal').modal('show');
			loadFeaturedProducts();
		}

		function addSelectedFeaturedProducts() {
			var selectedIds = selectedFeaturedProductIds();

			$('#featured-product-search-results .product-select:checked').each(function () {
				var id = parseInt($(this).val(), 10);
				if (selectedIds.indexOf(id) !== -1) {
					return;
				}
				if (selectedIds.length >= 12) {
					AIZ.plugins.notify('warning', '{{ translate('You can select a maximum of 12 featured products.') }}');
					return false;
				}

				var $row = $(this).closest('tr');
				var name = $.trim($row.find('.col span').first().text());
				var image = $row.find('img').attr('src');
				var $selectedRow = $('<tr>').attr('data-product-id', id);
				var $productCell = $('<td>');
				var $product = $('<div>').addClass('d-flex align-items-center');

				$product.append(
					$('<img>').addClass('size-48px img-fit mr-3').attr({ src: image, alt: name }),
					$('<span>').addClass('fs-13 fw-600').text(name)
				);
				$productCell.append(
					$product,
					$('<input>').attr({ type: 'hidden', name: 'featured_product_ids[]', value: id })
				);
				$selectedRow.append(
					$productCell,
					$('<td>').addClass('text-right').append(
						$('<button>').attr('type', 'button')
							.addClass('btn btn-icon btn-circle btn-sm btn-soft-danger remove-featured-product')
							.append($('<i>').addClass('las la-trash'))
					)
				);
				$('#selected-featured-products').append($selectedRow);
				selectedIds.push(id);
			});

			$('#featured-products-empty').toggleClass('d-none', selectedIds.length > 0);
			$('#featured-product-select-modal').modal('hide');
		}

		$(document).on('input', '#featured-product-search', loadFeaturedProducts);
		$(document).on('click', '.remove-featured-product', function () {
			$(this).closest('tr').remove();
			$('#featured-products-empty').toggleClass('d-none', selectedFeaturedProductIds().length > 0);
		});
	</script>
@endsection
