        ...
        <div class="col-10 col-sm-10 col-lg-10 mx-auto">
            <div class="aiz-titlebar text-left pb-5px">
                <div class="row align-items-center">
                    <div class="col-auto">
                        <h1 class="h3 fw-bold">{{ translate('All Colors') }}</h1>
                    </div>
                </div>
            </div>
            <div class="card">
                <div
                    ...
                    <div class="table-tabs-container">
                        <ul class="nav nav-tabs border-0 " id="myTab" role="tablist">
                            @foreach ($color_tabs as $color_tab)
                                <li class="nav-item" role="presentation">
                                    <button class="nav-link px-0 pb-15px fs-14 fw-500 {{ $loop->first ? 'active' : '' }}"
                                        ...
                                        id="{{ Str::slug($color_tab) }}-tab"
                                        onclick="changeTab(this, '{{ Str::slug($color_tab) }}')" role="tab"
                                        aria-controls="{{ Str::slug($color_tab) }}">
                                        {{ translate($color_tab) }}
                                    </button>
                                </li>
                            @endforeach
                    ...
                    <div class="">
                        @if (auth()->user()->can('add_color'))
                            <a href="javascript:void(0);" id="add_new_color"
                                class="position-relative overflow-hidden add-new-btn">
                                <span
                                    class="position-relative z-2 pr-15px fs-14 fw-500 text-blue label-text">{{ translate('Add New Color') }}</span>
                                <span
                            ...
                            <div class="flex-grow-1 mr-2">
                                <div class="input-group mb-0 border border-light px-3 bg-light rounded-1">
                                    ...
                                    <input type="text" class="form-control form-control-sm border-0 px-2 bg-transparent"
                                        id="search_input" name="search" placeholder="{{translate('Search Colors ...')}}">
                                </div>
                            </div>
...
@section('script')
    <script type="text/javascript">
        let currentTab = '{{ Str::slug($color_tabs[0]) }}';
        var searchTimer;
        ...
        function single_delete(colorId) {
            $.ajax({
                url: "{{ route('colors.destroy', ':id') }}".replace(':id', colorId),
                type: 'GET',
                success: function (response) {
                    if (response == 1) {
                        AIZ.plugins.notify('success', '{{ translate('Selected item deleted successfully') }}');
                        hideBulkActionModal();
                        getColors(currentTab);
                    }
                }
            });
        }
        ...
        function bulk_delete() {
            var data = new FormData($('#sort_colors')[0]);
            $.ajax({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                url: "{{route('bulk-color-delete')}}",
                type: 'POST',
                data: data,
                cache: false,
                contentType: false,
                ...
                success: function (response) {
                    if (response == 1) {
                        AIZ.plugins.notify('success', 'Selected colors Deleted successfully');
                        hideBulkActionModal();
                        getColors(currentTab);
                    }
                },
        ...
        function bulkDeleted() {
            if ($('.check-one:checked').length == 0) {
                AIZ.plugins.notify('danger', '{{ translate('Please select at least one color') }}');
                return;
            }

            showBulkActionModal();
            $('#confirmation-title').text('{{ translate('Delete Confirmation') }}');
            $('#confirmation-question').text('{{ translate('Are you sure you want to delete the selected colors?') }}');
        ...
        function singleDelete(colorId) {
            showBulkActionModal();
            $('#confirmation-title').text('{{ translate('Delete Confirmation') }}');
            $('#confirmation-question').text('{{ translate('Are you sure you want to delete the selected color?') }}');
            $('#conform-yes-btn').attr("onclick", "single_delete(" + colorId + ")");
            $('.confirmation-icon').addClass('d-none');
            $('#delete-confirm-icon').removeClass('d-none');
        }


        function getColors(slug, page = 1) {
            ...
            $.ajax({
                url: `{{ route('colors.filter') }}?page=${page}`,
                method: 'GET',
        ...
        $(document).on('click', '#store-new-color', function () {
            const btn = $(this);
            const name = $('#rightOffcanvas input[name="name"]').val().trim();
            const code = $('#rightOffcanvas input[name="code"]').val().trim();
            ...
            if (!code) {
                AIZ.plugins.notify('warning', 'Please fill code');
                return;
            }
            ...
            $.ajax({
                ...
                success: function (res) {
                    AIZ.plugins.notify('success', 'Color has been inserted successfully');
                    closeRightcanvas();
                    getColors(currentTab);
                },
                error: function () {
        ...
        $(document).on('click', '#update-color', function () {
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (res == 1) {
                        AIZ.plugins.notify('success', '{{ translate("Color updated successfully") }}');
                        closeRightcanvas();
                        getColors(currentTab);
                    }
                },
