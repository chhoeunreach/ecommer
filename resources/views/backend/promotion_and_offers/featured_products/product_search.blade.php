@forelse ($products as $product)
    <label class="d-flex align-items-center border rounded-2 p-3 mb-2 {{ $product->featured == 1 ? 'bg-soft-secondary' : '' }}">
        <span class="aiz-checkbox mr-3">
            <input type="checkbox" class="featured-product-select" value="{{ $product->id }}"
                @checked($product->featured == 1) @disabled($product->featured == 1)>
        ...
        @if ($product->featured == 1)
            <span class="badge badge-soft-success">{{ translate('Already Featured') }}</span>
        @endif
    </label>
@empty
