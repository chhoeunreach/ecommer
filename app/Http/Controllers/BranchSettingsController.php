...
class BranchSettingsController extends Controller
{
    ...
    public function update(Request $request)
    {
        $validated = $request->validate([
            ...
            'branches.*.lat' => ['nullable', 'numeric', 'between:-90,90'],
            'branches.*.lng' => ['nullable', 'numeric', 'between:-180,180'],
        ...
        $branches = collect($validated['branches'])
            ->map(function (array $branch) {
                return [
                    ...
                    'map' => trim($branch['map'] ?? ''),
                    'lat' => is_numeric($branch['lat'] ?? null) ? (float) $branch['lat'] : null,
                    'lng' => is_numeric($branch['lng'] ?? null) ? (float) $branch['lng'] : null,
