...
<section class="shadcn-section">
    <div class="container">
        <div class="text-center">
            <h1 class="shadcn-header-title">{{ translate('Our Computers') }}</h1>
            <p class="shadcn-header-subtitle">{{ translate('Explore our premium collection of computers designed just for you.') }}</p>
        </div>
                    ...
                    <div class="shadcn-card">
                        <div class="shadcn-card-img-wrapper">
                            @if ($computer->thumbnail_img != null)
                                <img src="{{ uploaded_asset($computer->thumbnail_img) }}" alt="{{ $computer->name }}" class="shadcn-card-img">
                            @else
                                <img src="{{ static_asset('assets/img/placeholder.jpg') }}" alt="{{ $computer->name }}" class="shadcn-card-img">
                            @endif
                        </div>
                        <div class="shadcn-card-content">
                            <h3 class="shadcn-card-title">{{ $computer->name }}</h3>
                            <div class="shadcn-card-desc">
                                {{ strip_tags($computer->description) }}
                            </div>
                            ...
                            <a href="{{ route('computers.show', $computer->id) }}" class="shadcn-btn-outline">{{ translate('View Details') }}</a>
                        </div>
                    </div>
                </div>
            @empty
                <div class="col-12 text-center py-5">
                    <h5 class="text-muted">{{ translate('No computers found.') }}</h5>
                </div>
            @endforelse
        </div>
