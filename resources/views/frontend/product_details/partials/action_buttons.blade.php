...
@if ((int) get_setting('product_detail_show_add_to_cart', 1) === 1)
    <button type="button" id="added_to_cart_btn"
        ...
        class="border-0 rounded-2 fs-14 fw-bold has-transition {{ $buttonPadding }} px-20px w-100 mb-2 mb-md-0 mr-0 @if ($showCustomButton) mr-md-2 @endif add-to-cart d-inline-flex align-items-center justify-content-center"
...
@if ($showCustomButton)
    <a href="{{ $customButtonUrl }}" @if ((int) get_setting('product_detail_custom_button_new_tab', 1) === 1) target="_blank" rel="noopener noreferrer" @endif
        class="d-flex align-items-center justify-content-center text-center border-0 rounded-2 fs-14 fw-bold hov-opacity-70 has-transition {{ $buttonPadding }} px-20px w-100"
